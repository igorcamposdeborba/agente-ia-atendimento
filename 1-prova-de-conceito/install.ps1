...
$temN1 = Get-ChildItem -Path $fonteDir -Filter 'n1*.xlsx' -ErrorAction SilentlyContinue | Select-Object -First 1
if ($temN1) {
    Write-Ok "Fontes ja presentes em: $fonteDir"
} else {
    $xlsx = Get-ChildItem -Path (Join-Path $projectRoot 'fonte') -Filter '*.xlsx' -ErrorAction SilentlyContinue
    if ($xlsx) {
        Copy-Item $xlsx.FullName $fonteDir -Force
        Write-Ok "Planilhas (ficticias) copiadas para: $fonteDir"
    } else {
        Write-Warn2 "Nenhum .xlsx para copiar. Coloque os exports (N1/Megazap/People/NPS) em: $fonteDir"
    }
}
Write-Info "Para dados reais, substitua os arquivos em $fonteDir (prefixos n1*/megazap*/people*/nps*)."
...
Write-Info "Instruction (regras/LGPD): $agentesDir\INSTRUCTION-lgpd.md"
Write-Info "Skill Preventivo        : $agentesDir\SKILL-preventivo.md"
Write-Info "Skill Pos-NPS           : $agentesDir\SKILL-pos-nps.md"
...
Proximos passos (manuais, no app do Claude):
  ...
  2. Confirme que 'millenium-agente-ia' aparece conectado com 5 tools.
