...
| `preventivo_contatos`      | **Fila completa e pronta do Preventivo** (gatilhos **A ∪ B**, deduplicada e priorizada por RFM) **já com o dossiê de cada cliente** (cadastro, produtos, valor soma NF, antiguidade/inatividade, gatilho(s), RFM, NPS, confiança, sinais). O **backend seleciona quem entra pelos gatilhos** — uma chamada traz tudo. **É a tool do documento** (a IA só formata, não escolhe gatilho nem chama `ficha_cliente` por cliente).                              |
| `clientes_inativos`        | Consulta do **subconjunto do gatilho A** (linhas-resumo). Para o documento, use `preventivo_contatos`.                                                                                                                                                                                                               |
| `clientes_para_contato`    | Consulta em linhas-resumo: A ou B (`preventivaVencida=true` = só B). Para o documento, use `preventivo_contatos`.                                                                                                                                                                                                    |
